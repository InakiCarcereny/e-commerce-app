...
export function ProductData() {
  ...
  return (
    <div className="ml-[300px] my-32 w-full flex flex-col items-center justify-center">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {
              images.map((image) => (
                <>
                <img className="rounded-xl h-24 w-auto" src={image.src} alt={products.title} key={image.src} />
                <img className="rounded-xl h-24 w-auto" src={image.src} alt={products.title} key={image.src} />
                <img className="rounded-xl h-24 w-auto" src={image.src} alt={products.title} key={image.src} />
                </>
              ))
      ...
      <button className="bg-[#007bff] text-white px-4 py-2 rounded-lg mt-8 font-semibold" onClick={handleGoBack}>
        Go back
      </button>
    </div>
  )
